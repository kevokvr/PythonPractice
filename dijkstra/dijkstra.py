...
class weighted_digraph:
    ...
    def dijkstra(self, start):
        for node in self.__nodes:# for all the nodes
            node.distance = float('inf')# set distance to float("inf")
            node.previous = None# set previous to None

        source = start.find(start)# set the source to the find of start
        source.distance = 0# set the source distance to 0

        todo = []
        todo.append(source)# create a todo set and add the source node

        result = []# create an empty result list

        while todo:# while there is something todo
            if node.distance < float('inf')# find the minimum distance node and remove it from todo
                min_node = node
            result.append([min_node.distance,min_node.value])# append its distance and value to the result list

            for edges in edges(min_node):# for each of the edges in the minimum node
                while min_node:# calculate the new distance to the all the adjacent nodes
                if min.distance < next(node):# if the new distance is less than the adjacent node's
                    min_node = node# existing distance
                    # update the adjacent node's distance and add it to the
                    todo.append(node)# todo list
